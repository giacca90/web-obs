...
globalThis.self.onmessage = (event) => {
    ...
    switch (type) {
        ...
        case 'addBitmap': {
            ...
            if (existing) {
                if (existing.type === 'bitmap')
                    existing.data.close();
                if (existing.latestFrame)
                    existing.latestFrame.close();
                if (existing.reader)
        ...
        case 'updateLayers':
            layersOrder = payload.layers.map((l) => l.id);
        ...
        case 'removeSource': {
            ...
            if (source) {
                if (source.type === 'bitmap')
                    source.data.close();
                if (source.latestFrame)
                    source.latestFrame.close();
                if (source.reader)
